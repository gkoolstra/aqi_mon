...
bool use_voc_sensor = true;
bool use_pm_sensor = true;
...
void setup()
{
  ...
  Wire.begin(); // temperature sensor
  sht.begin(0x44);    // temperature sensor I2C Address
  Wire.setClock(100000); // temperature sensor

  if (use_pressure_sensor) {
  if (!bmp.begin_I2C()) {   // bmp pressure sensor
    Serial.println("Could not find a valid BMP3 sensor, check wiring!");
    while (1);
  }

  // Set up oversampling and filter initialization
  bmp.setTemperatureOversampling(BMP3_OVERSAMPLING_8X); // bmp pressure sensor
  bmp.setPressureOversampling(BMP3_OVERSAMPLING_4X); // bmp pressure sensor
  bmp.setIIRFilterCoeff(BMP3_IIR_FILTER_COEFF_3); // bmp pressure sensor
  bmp.setOutputDataRate(BMP3_ODR_50_HZ); // bmp pressure sensor
  }

  if (use_voc_sensor) {
  if (! sgp.begin()){ // sgp30 gas sensor
    Serial.println("Sensor not found :("); // sgp30 gas sensor
    while (1);
  }
  Serial.print("Found SGP30 serial #"); // sgp30 gas sensor
  Serial.print(sgp.serialnumber[0], HEX); // sgp30 gas sensor
  Serial.print(sgp.serialnumber[1], HEX); // sgp30 gas sensor
  Serial.println(sgp.serialnumber[2], HEX); // sgp30 gas sensor
  }

  if (use_pm_sensor) {
  if (! aqi.begin_I2C()) {      // connect to the PM2.5 sensor over I2C
    Serial.println("Could not find PM 2.5 sensor!"); // PM2.5 sensor
    while (1) delay(10); // PM2.5 sensor
  }}
...
int counter = 0; // sgp gas sensor
void loop()
{
  PM25_AQI_Data data; // PM2.5 sensor
  CO2 = 0;
#ifdef SCD30
  CO2 = co2SCD30();
#endif
  ...
  if (CO2 > 0)
  {
    ...
    Serial.print(" ");
#endif
    sht.read(); // read temperature sensor and humidity
    
    Serial.print(CO2);
    Serial.print(",");
    Serial.print(sht.getTemperature(), 1); // temperature sensor
    Serial.print(",");
    Serial.print(sht.getHumidity(), 1); // temperature sensor
    if (use_pressure_sensor){
    Serial.print(",");
    Serial.print(bmp.pressure / 100.0, 1); // pressure sensor in hpa
    Serial.print(",");
    Serial.print(bmp.readAltitude(SEALEVELPRESSURE_HPA)); // pressure sensor, altitude estimate
    Serial.print(",");
    }
    ...
    Serial.print(sgp.TVOC, 1); // total VOC concentration in ppb
    Serial.print(",");
    Serial.print(sgp.eCO2); // equivalent CO2 in ppm 
    Serial.print(",");
    }
    ...
    //   return;
    // }
    // Serial.print(sgp.rawH2); // raw h2 measurement
    // Serial.print(",");
    // Serial.print(sgp.rawEthanol); // raw ethanol measurement  
    // Serial.print(",");  
    ...
    Serial.print(data.pm10_standard, 1); // pm 1.0, unit is ug/m^3, resolution = 1 ug/m^3
    Serial.print(",");
    Serial.print(data.pm25_standard, 1); // pm 2.5
    Serial.print(",");
    Serial.println(data.pm100_standard, 1); // pm 10.0
    }
    ...
    // Re-calibrate the baseline for the sgp gas sensor after every 30 tries
    counter++;
    if (counter == 30) {
      counter = 0;
